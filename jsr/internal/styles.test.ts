...
test("red() single color", function () {
    equal(c.red("foo bar"), "[31mfoo bar[39m");
});

test("red() replaces close characters", function () {
    equal(c.red("Hel[39mlo"), "[31mHel[31mlo[39m");
});

test("getColorEnabled() handles enabled colors", function () {
    equal(c.red("foo bar"), "[31mfoo bar[39m");
});
